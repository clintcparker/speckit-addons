...
- **A missing extension is now caught in the first seconds instead of at the step
  ...
  observed run of the send-it-checked variant lost its last three steps that way. It
  spent 93 minutes, built the feature, and ended with the work on a branch and
  nothing else: no pull request, the run lock held to its TTL — and a run status of
  "completed" over the top of it. The last steps are the ones this costs, because
  they are the ones with nothing after them to notice they are gone. Here that is
  `ship`, which is the whole point of the workflow.
  `requires:` cannot express the dependency (it recognizes only
