...
|---|---|---|
| [`yolo`](workflows/yolo/) | 0.9.0 | Full SDD cycle — `worktree` → `specify` → `plan` → `tasks` → `implement`, no review gates |
| [`send-it`](workflows/send-it/) | 0.10.0 | Spec to PR, unattended — `yolo` plus screenshots and `ship`, ending in an open pull request |
| [`send-it-checked`](workflows/send-it-checked/) | 0.11.0 | `send-it` plus staff review and QA, each with one fix-and-re-run pass |
...
| ID | Version | Source |
|---|---|---|
| [`screenshots`](extensions/screenshots/) | 0.4.0 | Hosted here |
